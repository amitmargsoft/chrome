...
var signatureData = {
	...
	copy: function(data) {
		...
		this.dllLocation = data.dllLocation;
...
document.addEventListener('DOMContentLoaded', function() {
	...
	chrome.runtime.sendMessage(
		...
		function(response) {
			...
			console.log('getBackgroundPage', background);
			console.log('appCurrentState ', background.window.appCurrentState);
			const popupMessageType = background.window.popupMessageType; //types of message from the popup that background script can handle
			const appStateEnum = background.window.StateEnum;
			console.log('EX ', background.window.StateEnum);
			const backgroundStoredSignatureData = background.window.storedSignatureData;
			let appCurrentState = background.window.appCurrentState;
			console.log('app CurrentState ', appCurrentState);
			...
			const dllfield = document.getElementById('dllpass-field');
			...
			useVisibleSignatureSwitchCheckbox.addEventListener('change', function() {
				...
				} else signatureData.visible = false;
			});
			...
			nextBtn.addEventListener('click', function() {
				...
				if (sections.currentSection == sections.section.selectSignatureTypeSection) {
					...
					if (
						...
						(signatureData.type == 'pades' && signatureData.visible == false)
					) {
			...
			document.querySelectorAll("input[type='radio'][name='vert-pos-radio']").forEach((el) =>
				el.addEventListener('click', function() {
					signatureData.verticalPosition = this.value;
				})
			);

			document.querySelectorAll("input[type='radio'][name='hor-pos-radio']").forEach((el) =>
				el.addEventListener('click', function() {
			...
			function selectSignatureTypeEvent() {
				...
				if (signatureData.type == 'pades') {
					...
					if (useVisibleSignatureSwitchCheckbox.checked) useVisibleSignatureSwitchCheckbox.click();
         ...
         */
			function checkCurrenState() {
				...
				if (
					...
					appCurrentState == appStateEnum.downloadFile ||
					appCurrentState == appStateEnum.info
				) {
					console.log('LOADING');
					// sections.changeSection(sections.section.loadingSection);
						...
						chrome.tabs.query(
							...
							function(tab) {
								if (tab[0].url == backgroundStoredSignatureData.signatureData.tabUrl) {
									...
									console.log(
										'Stored data in background belong to a different document. Clear stored data.'
									);
									clearData();
								}
							}
         ...
         */
			function sign(tabData) {
				...
				signatureData.dllLocation = dllfield.options[dllfield.selectedIndex].value;
         ...
         */
			function getTabData(callback) {
				...
				if (
					...
					!signatureData.filename.startsWith('http') &&
					!signatureData.filename.startsWith('file')
				) {
         ...
         */
			function updateSignatureFieldList(fieldsData) {
				...
				page_input.addEventListener('input', (e) => {
					...
					if (e.target.checkValidity()) nextBtn.disabled = false;
